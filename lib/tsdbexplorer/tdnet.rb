  ...
  module TDnet
   ...
   def TDnet.parse_raw_message(message)
     ...
     structure = case result[:message_type]
       ...
       when "0104"
         { :delete => [ :spare_1, :spare_2 ], :format => [ [ :zone, 1 ],  [ :area_code, 2 ],  [ :alert_number, 8 ],  [ :record_type, 2 ],  [ :message_date, 6 ],  [ :message_timestamp, 4 ],  [ :current_train_id, 10 ],  [ :stanox_a, 5 ],  [ :spare_1, 18 ],  [ :cancel_reason_code, 2 ],  [ :cancel_type, 1 ],  [ :stanme_a, 9 ],  [ :reason_description, 10 ],  [ :attributed_flag, 1 ],  [ :rolled_up_flag, 1 ],  [ :original_train_id, 10 ],  [ :spare_2, 106 ] ] }
       when "0003"
